	...
	function tryNextDevice() {
		...
		device.open({stopBits: 0, bitRate: 9600, ctsFlowControl: 0}, function () {
			device.set_receive_handler(function (data) {
				...
				console.log('Dado Recebido Raw: ' + messageBuffer);
				

				var aaa = "";
				var separator = "";
				for (var i = 0;i<messageBuffer.length; i++) {
					aaa += separator + messageBuffer.charCodeAt(i);
					separator = " ; "; 
				}

				console.log('Cod. Recebido Raw: ' + aaa);
				
				while (messageBuffer.indexOf("\n") >= 0) {
					message = messageBuffer.substr(0, dataString.indexOf("\n"));
					messageBuffer = messageBuffer.substr(dataString.indexOf("\n") + 1);
